...
func (p *PostgresqlDatabase) GetRawDbSizeMb(
	...
	conn, cleanup, err := p.connect(ctx, logger, encryptor)
	if err != nil {
...
func (p *PostgresqlDatabase) EncryptSensitiveFields(
	...
	if p.SslCa != "" {
		encrypted, err := encryptor.Encrypt(p.SslCa)
		if err != nil {
			return err
		}
		p.SslCa = encrypted
	...
	if p.SslCert != "" {
		encrypted, err := encryptor.Encrypt(p.SslCert)
		if err != nil {
			return err
		}
		p.SslCert = encrypted
	...
	if p.SslKey != "" {
		encrypted, err := encryptor.Encrypt(p.SslKey)
		if err != nil {
			return err
		}
		p.SslKey = encrypted
...
func testSingleDatabaseConnection(
	...
	conn, cleanup, err := postgresDb.connect(ctx, logger, encryptor)
	if err != nil {
		return err
	}
	defer cleanup()
...
func (p *PostgresqlDatabase) connect(
	...
	if p.SslMode != "" && p.SslMode != "disable" {
		...
		if p.SslCa != "" {
			ca, err := decryptFieldIfNeeded(p.SslCa, encryptor)
			if err != nil {
		...
		if p.SslCert != "" && p.SslKey != "" {
			cert, err := decryptFieldIfNeeded(p.SslCert, encryptor)
			if err != nil {
				return nil, nil, fmt.Errorf("failed to decrypt SSL cert: %w", err)
			}

			key, err := decryptFieldIfNeeded(p.SslKey, encryptor)
			if err != nil {
...
func decryptPasswordIfNeeded(
	...
	return decryptFieldIfNeeded(password, encryptor)
}
